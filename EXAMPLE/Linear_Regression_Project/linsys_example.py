...
from plane_example import Plane
...
class LinearSystem(object):
    ...
    def compute_rref(self):
        tf = self.compute_trianglar_form()
    ...
    def multiply_coefficient_and_row(self, coefficient, row):
        ...
        new_vector = n.scalar(coefficient)
        new_constant = k * coefficient

        self[row] = Plane(normal_vector=new_vector, constant_term=new_constant)
    ...
    def compute_solution(self):
        try:
            return self.do_gaussian_elimination_and_parametrize_soluiton()
    ...
    def do_gaussian_elimination_and_parametrize_soluiton(self):
        rref = self.compute_rref()

        rref.raise_exception_if_contradictory_equation()
        # rref.raise_exception_if_too_few_pivots()
        direction_vectors = rref.extract_direction_vector_for_parametrization()
...
class Parametrization(object):
    ...
    BASEPT_AND_DIR_VECTOR_MUST_BE_IN_SAME_DIM_MSG = ('The basepoint and direction vectors should all live in the same dimendion')
    def __init__(self, basepoint, direction_vectors):
        
        self.basepoint = basepoint
        self.direction_vectors = direction_vectors
        self.dimendion = self.basepoint.dimension

        try:
            for v in direction_vectors:
                assert v.dimension == self.dimendion

        except AssertionError:
            raise Exception(self.BASEPT_AND_DIR_VECTOR_MUST_BE_IN_SAME_DIM_MSG)
